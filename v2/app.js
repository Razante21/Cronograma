...
=======
  ...
  sb.auth.onAuthStateChange(async (event, session) => {
    if (_sessionApplying || event === 'TOKEN_REFRESHED') return;
    if (session?.user) {
...
>>>>>>> e92589f8dd4c639952bcf0b9ddcd7283769e429a
  });
  const { data: { session } } = await sb.auth.getSession();
  if (session?.user) await applySession(session.user);
  else showLanding();
...
document.getElementById('auth-submit').addEventListener('click', async () => {
  ...
  try {
    if (authMode==='login') {
      ...
      const {data,error} = await sb.auth.signUp({email,password:pass,options:{
